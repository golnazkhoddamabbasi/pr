...
compony :{
 ...
 useEffect(()=>{
     const Data = async()=>{
         try{
            ...
            setState({...state ,compony:ComponyData })
        }catch (err){
              console.log(err)
  }
    }
 ...
 setState({...state,compony:{...state.compony,[event.target.name]: [event.target.value] ,
}
 })  
 ...
 const sumbit = async(event) =>{
     ...
     try{
     const {data}= await updateCompony(state.compony , companyId)
